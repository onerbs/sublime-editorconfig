...
def parse_file(file_root, file_name):

	ec_file = path.join(file_root, '.editorconfig')
	options = {}

	if not path.exists(ec_file):
		prent_folder = path.dirname(file_root)
		return options if prent_folder == file_root else parse_file(prent_folder, file_name)

	aplicable = False
	lines = list(map(lambda s: s.lower().strip(), open(ec_file).readlines()))
