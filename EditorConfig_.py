...
def dispatch(event, view):
	...
	if event == '_start_' or event == '_load_':
		...
		if (
			...
			print('Updated charset from '+encoding+' to '+new_encoding)
	...
	if event == '_save_':
		...
		if insert_final_newline == 'true':
			...
			print('Run: normal_final_newlines')
		else:
			view.run_command('remove_final_newlines')
			print('Run: remove_final_newlines')
...
def _read(file_path, file_name):
	...
	if not path.exists(ec_file):
		...
		return dict(editorconfig='None') if prent_path == file_path else _read(prent_path, file_name)
	...
	for line in lines:
		...
		if line[0] == '[':
			sectname = line[1:-1]
			# print('pattern: {}'.format(sectname))
			aplicable = fnmatch(file_name, sectname)
			continue
